...
model_mapping = {
    ...
    "gpt-4o-mini": os.getenv("AZURE_OPENAI_COMPLETION_MODEL"),
}
...
def handle_other(file) -> str:
    try:
        ...
        out_text = "".join(
            chunk.metadata.text_as_html if chunk.category == "Table" else chunk.text
            for chunk in chunks
        )
...
def extract_text(uploaded_file, use_local_pdf=False) -> str:
    try:
        ...
        file_handlers = {
            ...
            "application/pdf": (
                handle_pdf_locally if use_local_pdf else handle_pdf_remotly
            ),
        }
...
def get_improved_prompts(query, temperature, model):
    ...
    prompt = (
        f"Could you help me improve the following prompt to ensure it elicits higher-quality responses from the GPT model? "
...
def get_prompt_from_template(user_prompt_template, question, document):
    return user_prompt_template.replace("{question}", question).replace(
        "{document}", document
    )
